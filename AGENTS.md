...
- Command Center create/edit flows (Finance ledger, Install Ops, Contacts Create lead, similar ops tools): prefer a primary button that opens a modal (`<dialog>`), not an always-open form that pushes the list below the fold; keep dialogs inside `<main>` so `spa-router.js` retains them on navigation.
- Command Center ops pages should open on the working chrome (KPIs, tabs, action bar) — skip marketing `ops-page-head` blocks (SC eyebrow, long lede, mission chips) unless the user asks for them.
- Keep the Compliance checklist off the customer quote sheet and PDF; permit / electrician / net-metering gates stay on the Jobs board (and Install Ops), not on homeowner paperwork.
- Manual Create lead: phone is the only required field; name and other fields stay optional.
...
- Local founder-auth against Wrangler is driven by `funnel/.dev.vars` (`MASTER_EMAIL` / `MASTER_PASSWORD` / `FOUNDER_PASSWORD`); repo-root `.env` (`EMAIL` / `PASSWORD`) is only useful if that account exists in the local D1 `users` table — never commit either file or paste values into source/docs.
- Cloudflare Pages serves `funnel/` as the site root (Functions under `funnel/functions/` deploy with the Pages project — no separate Worker deploy); the repo-root engineering docs are a separate static site.
...
- Founder titles: Benjie Malinao — sales & marketing head; Jundhel Cabradilla — president, operations & engineering head; Ace Denver Corpin — administration head; Jethro Abatias — finance & accounting head (Funnel v2 card: "Support & systems", IT/network bio pending his sign-off). On-island founders (Jundhel, Ace, Jethro) may use "From Biliran"; Benjie's card should not.
- Goals (`goals.html` / `/api/goals`): countable founder outcomes (leads, sold, deposits, surveys, manual) with start/end dates, owner (session), assignee, board + timeline, urgency colors; dashboard “Goals on track” + Goals panel. Replaces the old Jobs “Company tasks” tab.
- Command Center home is the SC-00 dashboard (`funnel/internal/index.html` + `assets/dashboard.{css,js}`): Leads metric (total + new in last 7 days), death clock, goals-on-track, cash, and a "needs you today" queue ranked by consequence — all derived from the live APIs, never stored strings. The sidebar lists every tool as a **permanently narrow icon rail** (82px on desktop — icon over a short label, no hover-expand and no pin; full names are `title` attributes, and below 981px it becomes the wide mobile drawer): Dashboard, Leads, Mail, Quotes, Jobs, Install ops, Finance, Documents, Meetings, Captures, then a "Strategy" group (Actions, Goals, Notes, Lab, Market, Eng file, Legal, All apps), with Log and Team in the footer above theme + log out. Seventeen entries do not fit a fixed rail, so `.cc-sb-nav` scrolls with a visible thumb and a fade over its bottom edge. Every entry carries `data-section`, so `assets/section-visibility.js` drops the ones an account may not open — the rail lists sections, so it must be filtered like the Apps grid. `funnel/internal/apps.html` (`assets/apps.{css,js}`) remains the full grid, with a line on what each tool is for. Shell chrome and the rail live in `assets/shell.css` (formerly `hub-home.css`, renamed when the launcher grid moved out), and `body.cc-shell` is toggled per route by `spa-router.js`. That router never re-injects the dashboard's own styles or scripts when swapping home: they are this document's own head, and re-adding the inline `<style>` after `shell.css` would flip every rule the two share (the rail width among them).
- Contacts (`leads.html` / `/api/leads`): founders can create leads (`POST`, source `manual`), edit contact fields from the drawer, and attach files via `documents.lead_id`; those contact attachments are excluded from the Documents library list (`WHERE lead_id IS NULL`). Documents APIs (`/api/documents`, `/api/document-file`) are shared with the Contacts section in `_pages.js` so uploads work without opening the library.
- Homeowner quote attachment: `docs/client-quote-companion.md` (packages, brownout honesty, BILECO net metering / RA 9513; no Biliran cash rebate). Compliance checklist is internal-only — not printed on the customer quote or PDF.
