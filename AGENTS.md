...
- Command Center home is the SC-00 dashboard (`funnel/internal/index.html` + `assets/dashboard.{css,js}`): Leads metric (total + new in last 7 days), death clock, traction floor, cash, and a "needs you today" queue ranked by consequence — all derived from the live APIs, never stored strings. The sidebar lists every tool as a **permanently narrow icon rail** (82px on desktop — icon over a short label, no hover-expand and no pin; full names are `title` attributes, and below 981px it becomes the wide mobile drawer): Dashboard, Leads, Mail, Quotes, Jobs, Install ops, Finance, Documents, Meetings, Captures, then a "Strategy" group (Actions, Notes, Lab, Market, Eng file, Legal, All apps), with Log and Team in the footer above theme + log out. Seventeen entries do not fit a fixed rail, so `.cc-sb-nav` scrolls with a visible thumb and a fade over its bottom edge. Every entry carries `data-section`, so `assets/section-visibility.js` drops the ones an account may not open — the rail lists sections, so it must be filtered like the Apps grid. `funnel/internal/apps.html` (`assets/apps.{css,js}`) remains the full grid, with a line on what each tool is for. Shell chrome and the rail live in `assets/shell.css` (formerly `hub-home.css`, renamed when the launcher grid moved out), and `body.cc-shell` is toggled per route by `spa-router.js`. That router never re-injects the dashboard's own styles or scripts when swapping home: they are this document's own head, and re-adding the inline `<style>` after `shell.css` would flip every rule the two share (the rail width among them).
